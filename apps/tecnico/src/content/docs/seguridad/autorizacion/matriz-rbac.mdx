...
1. Para recursos globales, los candidatos son permisos de roles globales y `grants.direct_permissions`; el guard aplica además la regla del recurso y del rol cuando corresponda.
...
3. Los permisos de club salen solo de `active_assignment`; no se unen todas las asignaciones del usuario para una request.
4. `grants.direct_permissions` no sustituye scope, rol requerido ni metadata del recurso: es una fuente de candidatos globales para `effective.permissions`.
...
| `notifications:club` | Consultar destinos o enviar a un club | `active_assignment` | permiso efectivo + asignación activa exacta; el servicio valida nuevamente el destino | Admin |
| `audit:read` | Consultar bitácora de auditoría | `global` | exige `super-admin` y permiso global; no aplica scope territorial | Admin |
| `permissions:read` | Consultar RBAC | `global` | permiso global | Admin |
| `permissions:assign` | Cambiar permisos | `global` | permiso global | Admin |
| `roles:read` | Ver roles | `global` | permiso global | Admin |

## Consulta y mutación

La matriz separa consultas de mutaciones: que un permiso permita consultar una superficie no implica permiso para crear, editar, revocar o eliminar en ella. Cada ruta declara su propio requisito y recurso.

### Consulta global de auditoría

`audit:read` es un permiso global, pero no habilita el visor por sí solo: la ruta exige también el rol `super-admin`. No tiene alcance territorial.

### Notificaciones de club

`notifications:club` no se resuelve como acceso global en Stage 1. Tanto `POST /notifications/club/:instanceType/:instanceId` como `GET /notifications/targets/club` usan el recurso `active_assignment`; el servicio valida la asignación exacta contra el destino solicitado.
...
| Post-registro (completar) | `POST /step-1/complete`, `POST /step-2/complete`, `POST /step-3/complete` | `registration:complete` _(sin fallback)_ | ownership o permiso global dedicado; terceros quedan en modo administrativo mínimo | Verificado runtime |
...
- OR transicional vigente en backend: permiso fino de familia o fallback legacy de la familia `users:*` (`users:read_detail` para lectura, `users:update` para escritura). Excepción: `registration:complete` no tiene fallback legacy.
- `registration:complete` es un permiso global dedicado asignado a roles de campo (super-admin, admin, assistant-lf, director-lf y equivalentes union/dia por herencia). El owner siempre puede completar su propio registro sin este permiso.
...
- Excepción mínima vigente: `post_registration` de terceros mantiene lectura administrativa mínima y completion con `registration:complete`, sin feedback sensible detallado.
