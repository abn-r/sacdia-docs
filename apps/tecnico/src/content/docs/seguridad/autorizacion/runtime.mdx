...
- la autenticación base depende de **Better Auth** self-hosted (módulo `src/better-auth/` en sacdia-backend);
- SACDIA firma su propio JWT **HS256** con `BETTER_AUTH_SECRET` (Option C: Better Auth autentica, SACDIA firma);
...
Vive en `sacdia-backend/src/better-auth/` (`BetterAuthModule` + `BetterAuthService`).

Responsabilidades actuales:
- autenticar credenciales con email y password;
- soportar OAuth con Google y Apple;
- gestionar el ciclo de sesión Better Auth (sign-in / sign-out / refresh);
- exponer endpoints internos consumidos por el módulo `auth` de SACDIA;
- el JWT emitido al cliente final es firmado por SACDIA (HS256) usando `BETTER_AUTH_SECRET`, no por un proveedor externo.
...
2. backend devuelve una URL de OAuth Better Auth;
...
5. backend resuelve el usuario contra Better Auth y crea/replica usuario local en Prisma si es primera vez;
6. SACDIA emite su propio JWT HS256 al cliente;
...
Estado actual soportado: MFA con TOTP implementado en NestJS (servicio propio, no provider externo).
...
- pueden aceptar `x-refresh-token` opcional para bind de sesión cuando el flujo lo requiere;
...
Estado actual:
- `POST /api/v1/auth/register` crea usuario en Better Auth y réplica local en Prisma;
...
- JWT HS256 (firmado por SACDIA con `BETTER_AUTH_SECRET`) autentica identidad;
