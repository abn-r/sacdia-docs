...
- `grants.direct_permissions`: permisos asignados directamente al usuario; son candidatos globales que el backend incorpora al resolver `effective.permissions`.
...
- `effective.permissions`: permisos listos para gating en clientes; combina permisos de roles globales aplicables, `grants.direct_permissions` y, cuando existe, la asignación de club activa.
...
Los identificadores de roles usan guiones: `super-admin` y `assistant-admin`. Los clientes no deben convertirlos a `snake_case` al consumir el contrato.

## Guard global fail-closed

En una ruta autenticada, `PermissionsGuard` exige `@RequirePermissions(...)` y `@AuthorizationResource(...)`, salvo una excepción explícita mediante `@Public` o `@SkipPermissions`. Si falta el requisito o el recurso, falla cerrada con `GUARD_RBAC_MISCONFIGURATION`; no se interpreta como acceso permitido.

Los permisos directos solo aportan candidatos globales. No reemplazan el alcance territorial, un rol adicional requerido ni el recurso declarado por la ruta.

## Estado de permisos y revocación

Un permiso directo desactivado se filtra de la resolución efectiva. En cambio, el runtime actual no garantiza revocación inmediata de grants por rol cuando se desactiva un permiso: es un gap conocido, no una promesa del contrato.
...
- excepción: `registration:complete` es un permiso global dedicado sin fallback legacy. Asignado a roles de campo (super-admin, admin, assistant-lf, director-lf y equivalentes union/dia por herencia). El owner siempre puede completar su propio registro sin este permiso.
...
- `POST /users/:userId/post-registration/step-{1,2,3}/complete` requiere `registration:complete` (permiso global dedicado, sin fallback a `users:update`). Roles asignados: super-admin, admin, assistant-lf, director-lf y equivalentes union/dia por herencia;
