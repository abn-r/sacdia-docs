...
- `authorization.grants`, incluido `grants.direct_permissions`
- `authorization.active_assignment`
- `authorization.effective`

Regla:

- `effective.permissions` es la fuente operativa para habilitar acciones en cliente.
- `grants.direct_permissions` es una lista de permisos directos del usuario y contribuye como candidato global a `effective.permissions`.
...
- excepción: los endpoints `POST /step-{1,2,3}/complete` ahora usan `registration:complete` (permiso global dedicado) en lugar de `post_registration:update` con fallback a `users:update`. Roles asignados: super-admin, admin, assistant-lf, director-lf (y equivalentes union/dia por herencia). El owner siempre puede completar su propio registro;
...
| `audit:read` | Consultar la bitácora global de auditoría; el visor exige además `super-admin` y no tiene alcance territorial |
...
| `notifications:club` | Consultar destinos o enviar notificación al club de la `active_assignment`; no es un permiso global de Stage 1 |
...
La baja lógica de un permiso impide que un permiso directo inactivo contribuya a la autorización resuelta. Sin embargo, existe un gap runtime: desactivar un permiso NO garantiza la revocación inmediata de los grants por rol ya resueltos. No documentar ese comportamiento como garantía hasta cerrar el gap.
