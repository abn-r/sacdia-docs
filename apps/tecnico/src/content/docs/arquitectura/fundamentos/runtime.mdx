...
- **Backend**: NestJS 11.x + Node.js 22.x LTS + TypeScript 6.x
- **Admin web**: Next.js 16 (App Router + `proxy.ts`) + TypeScript + Tailwind v4 + shadcn/ui
- **App móvil**: Flutter 3.27+ / Dart 3.6+
- **Datos**: PostgreSQL 16 en **Neon** (3 branches) + Prisma 7
- **Auth**: Better Auth self-hosted (HS256 JWT firmado con `BETTER_AUTH_SECRET`)
- **Storage**: Cloudflare R2 (presigned URLs)
- **Realtime**: Firebase FCM (silent multicast → cache invalidation)
...
- **Base URL**: `/api/v1`
- **Total**: 362 endpoints (live reference; ver `docs/api/ENDPOINTS-LIVE-REFERENCE.md`)

### Módulos API documentados

El runtime documenta actualmente estos módulos:

- `auth` (login, refresh, logout, me, OAuth Google/Apple, MFA, sessions)
- `users` (perfil, foto, sub-recursos clínicos)
- `activities` (incluye `is_joint` para actividades conjuntas)
- `admin` (geography, reference, users, catalogs, system-config)
- `audit-logs` ⭐ nuevo
- `camporees`
- `catalogs` (24 catálogos i18n-aware)
- `certifications`
- `classes`
- `club-roles`
- `clubs` (overview, leadership, history)
- `evidence-review` ⭐ nuevo
- `fcm-tokens`
- `finances`
- `folders` (carpetas de evidencia anuales)
- `health`
- `honors`
- `inventory`
- `materiales` ⭐ nuevo (catalogo, ordenes, comprobantes, inventario, configuracion)
- `notifications`
- `rankings` ⭐ nuevo
- `resources` ⭐ nuevo (presigned R2 upload)
...
- PostgreSQL 16 en Neon (3 branches: development/staging/production);
- Prisma 7 como capa ORM;
...
El schema de persistencia contiene 152 modelos (Prisma). Se categorizan así:
...
- **Better Auth (self-hosted)** para autenticación + OAuth Google/Apple;
- **PostgreSQL en Neon** (3 branches) para persistencia;
- **Cloudflare R2 (S3-compatible)** como storage de archivos (presigned URLs);
- **Firebase Cloud Messaging** para push notifications + realtime cache invalidation (silent multicast);
- **Redis** (BullMQ + cache-aside + throttler distribuido + blacklist tokens + sessions);
- **Sentry** para monitoreo y error tracking (backend y mobile);
- **Render.com** como hosting para backend;
- **Vercel** como hosting para admin web.
