...
### 6.1 Crear / Editar -> Flujo por paginas (default)

**REGLA**: Crear y Editar usan paginas dedicadas (`/new` y `/[id]`) por default.

**Excepcion** (DS §6.1.1, decision 2026-05-11):
- Dialog SE PERMITE solo si: ≤4 campos planos, sin relaciones, sin uploads, sin tabs.
- Cualquier flujo con >4 campos O con relaciones / imagenes / tabs DEBE usar pagina dedicada.

**Helpers existentes**:
- `<CatalogCrudPage>` — pagina CRUD generica con tabla + form
- `<PhaseECatalogCrudPage>` — wrapper para los 24+ catalogos i18n-aware (gestiona `TranslationsTabsField` + `<entity>_translations` overlay automaticamente)
