...
> **Nota**: Los actividades pueden ser conjuntas (`is_joint: true`). Una actividad conjunta crea multiples `activity_instances`, una por seccion participante. El backend valida autorizacion contra todas las secciones via `PermissionsGuard`.
...
export function useClubActivities(clubId: number, filters?: {
  ...
  return useQuery({
    ...
    queryFn: () => apiRequest<Activity[]>(url),
  });
}
...
export function useFinancialSummary(
  ...
  return useQuery({
    ...
    queryFn: () => apiRequest<FinancialSummaryEnvelope>(
