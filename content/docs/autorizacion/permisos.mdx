...
- excepción: los endpoints `POST /step-{1,2,3}/complete` ahora usan `registration:complete` (permiso global dedicado) en lugar de `post_registration:update` con fallback a `users:update`. Roles asignados: super_admin, admin, assistant-lf, director-lf (y equivalentes union/dia por herencia). El owner siempre puede completar su propio registro;
